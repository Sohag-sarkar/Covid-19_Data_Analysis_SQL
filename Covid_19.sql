...
select location, population,max(total_deaths) as Total_death, max((total_deaths/population)*100) as Percentage_death_rate
from project.covid_deaths1
where continent is not null
group by location
order by Total_death desc;
...
-- Use CTE to calculate Rolling people vaccination
...
-- Temp table to calculate Vaccination of People
...
(
select dea.continent,dea.location,str_to_date(dea.date), dea.population,vac.new_vaccinations,vac.people_vaccinated,
sum(new_vaccinations) over (partition by dea.location Order by dea.location) as Rolling_People_vaccinated
from project.covid_deaths1 dea join project.covid_vaccination1 vac
on dea.location= vac.location and str_to_date(dea.date)= str_to_date(vac.date)
where dea.continent is not null and vac.people_vaccinated is not null
order by dea.location
);
...
as select dea.continent,dea.location,dea.date, dea.population,vac.new_vaccinations,vac.people_vaccinated,
sum(new_vaccinations) over (partition by dea.location Order by dea.location) as Rolling_People_vaccinated
from project.covid_deaths1 dea join project.covid_vaccination1 vac
on dea.location= vac.location and dea.date= vac.date
